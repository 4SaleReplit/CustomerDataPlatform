    ...
    try {
      ...
      if (isSendNow) {
        ...
        if (reportData.emailTemplate?.templateId) {
          ...
          // Fallback to simple text processing with anti-spam improvements
...
<head>
    ...
    <title>Analytics Report from 4Sale</title>
    <style>
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif; margin: 0; padding: 0; background-color: #f8f9fa; }
        ...
        .meta { background: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0; font-size: 14px; color: #666; }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1 style="margin: 0; font-size: 24px;">4Sale Analytics Platform</h1>
            <p style="margin: 10px 0 0 0; opacity: 0.9;">Business Intelligence Report</p>
        </div>
        <div class="content">
            <h2 style="color: #333; margin-bottom: 20px;">Report: ${reportData.name}</h2>
            <p style="line-height: 1.6; color: #555; margin-bottom: 20px;">${emailContent}</p>
            
            <div class="meta">
                <strong>Report Details:</strong><br>
                Generated: ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} at ${new Date().toLocaleTimeString()}<br>
                Presentation ID: ${reportData.presentationId}<br>
                Requested by: 4Sale Analytics System
            </div>
            
            <div style="text-align: center;">
                <a href="https://bfe134db-1159-40e3-8fad-f3dbf1426e39-00-2pzuierkpxkak.janeway.replit.dev" class="btn">
                    Access Analytics Dashboard
                </a>
            </div>
            
            <p style="color: #777; font-size: 14px; margin-top: 30px;">
                This report was automatically generated by the 4Sale Analytics Platform. 
                For questions or support, please contact our team.
            </p>
        </div>
        <div class="footer">
            <p><strong>4Sale Analytics Platform</strong></p>
            <p>Professional Business Intelligence & Reporting</p>
            <p>© ${new Date().getFullYear()} 4Sale Technologies. All rights reserved.</p>
            <p>You received this email because you are subscribed to analytics reports.</p>
        </div>
    </div>
</body>
</html>`;
        ...
        // Send email immediately using Gmail SMTP with anti-spam headers
        const emailData = {
          to: reportData.recipientList,
          cc: reportData.ccList || [],
          bcc: reportData.bccList || [],
          subject: reportData.emailTemplate?.subject || `Analytics Report: ${reportData.name}`,
          html: emailHtml,
          text: `Analytics Report: ${reportData.name}\n\n${reportData.emailTemplate?.customContent || 'Your report is ready.'}\n\nGenerated: ${new Date().toLocaleString()}\n\nView Dashboard: https://bfe134db-1159-40e3-8fad-f3dbf1426e39-00-2pzuierkpxkak.janeway.replit.dev`,
